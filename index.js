...
const col_names = ["Region","Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];

function verify_table(s) {
  const table = document.getElementById(s);
  var i = 0;
  for (var j = 0, col; col = table.rows[i].cells[j]; j++) {
    if (col_names[j] != col.textContent) {
      col.bgColor = "red";
    }
    else {
      col.bgColor = "#33cc33";
...
var process_wb = (function() {
	...
	return function process_wb(wb) {
		...
		wb.SheetNames.forEach(function(sheetName) {
			var htmlstr = XLSX.utils.sheet_to_html(wb.Sheets[sheetName],{editable:true,id:"datatable"});
